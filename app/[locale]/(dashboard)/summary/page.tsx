...
export default async function SummaryPage() {
  ...
  const [
    ...
    supabase
      .from("campaigns")
      .select(
        "id, name, status, budget_total, budget_spent, customers, leads, revenue, project:projects(name)",
      ),
    supabase
      .from("contents")
  ...
  const roi =
    totalSpent > 0
      ...
      : null;
  const cac =
    totalCustomers > 0 ? Math.round(totalSpent / totalCustomers) : null;
  const ltv =
    totalCustomers > 0 ? Math.round(totalRevenue / totalCustomers) : null;
  ...
  // ── помесячный контент (последние 6 мес) ────────────────────
  ...
  const metrics = [
    ...
    {
      label: "Выручка с кампаний",
      val: totalRevenue > 0 ? `${fmtMoney(totalRevenue)} сум` : "—",
      Icon: Coins,
    },
  ];
    ...
    <div className="p-7 md:p-8">
      ...
      <div className="flex items-end justify-between mb-7">
        <div>
          ...
          <h1 className="text-[30px] font-semibold tracking-tight text-tx-1 mt-2">
            {monthTitle}
          </h1>
        ...
        <Link
          ...
          className="inline-flex items-center gap-2 text-[13px] font-medium px-3.5 py-2.5 rounded-[10px] border border-line text-tx-2 hover:text-tx-1 hover:border-line-strong transition-colors"
        >
      ...
      {!hasData && (
        <div className="ui-surface flex flex-col items-center justify-center text-center py-16 px-6 mb-4">
          ...
          <div className="text-[15px] font-semibold text-tx-1">
            Пока нет данных по экономике
          </div>
          <p className="text-[12.5px] text-tx-3 mt-1.5 max-w-[360px] leading-relaxed">
            Создай кампанию и внеси бюджет и результаты — здесь появятся ROI,
            CAC и LTV по всему бизнесу.
          </p>
          <Link
            href={`/${locale}/campaigns`}
            className="mt-4 inline-flex items-center gap-2 text-[13px] font-medium px-3.5 py-2.5 rounded-[10px] bg-accent text-on-accent hover:opacity-90 transition-opacity"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5 mb-4">
        {metrics.map((m) => (
          <div key={m.label} className="ui-surface p-4">
            ...
            <div
              className={`ui-num text-[24px] font-semibold leading-none ${
        ...
        <div className="ui-surface p-4">
          ...
          <div className="ui-num text-[24px] font-semibold leading-none text-tx-1">
            {fmtMoney(totalSpent)}
            <span className="text-[14px] text-tx-2 font-medium">
              {" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4 mb-4">
        ...
        <div className="ui-surface p-5">
          <h2 className="text-[14.5px] font-semibold text-tx-1 mb-3">
            Юнит-экономика
          </h2>
          <div className="flex items-baseline gap-2 mb-2">
            <span className="ui-num text-[34px] font-semibold text-accent">
              {ratio != null ? `${ratio}×` : "—"}
            </span>
          ...
          <p className="text-[11.5px] text-tx-3 leading-relaxed mb-4">
            ...
            {ratio != null && Number(ratio) >= 3
              ? "Контент окупается с запасом."
              : "Появится, когда внесёшь результаты кампаний."}
          </p>

          <div className="ui-label mb-1">Контент</div>
          <div className="flex items-center justify-between py-2.5 border-b border-line">
            <span className="flex items-center gap-2 text-[13px] text-tx-1">
              <FileText size={14} className="text-tx-3" /> Опубликовано
            </span>
            <span className="ui-num text-[14px] font-semibold text-tx-1">
              {publishedCount || 0}
            </span>
          </div>
          <div className="flex items-center justify-between py-2.5 border-b border-line">
            <span className="flex items-center gap-2 text-[13px] text-tx-1">
              <CalendarClock size={14} className="text-tx-3" /> Запланировано
            </span>
            <span className="ui-num text-[14px] font-semibold text-tx-1">
              {scheduledCount || 0}
            </span>
          ...
          <div className="flex items-center justify-between py-2.5">
            <span className="flex items-center gap-2 text-[13px] text-tx-1">
              <Coins size={14} className="text-tx-3" /> Всего создано
            </span>
            <span className="ui-num text-[14px] font-semibold text-tx-1">
              {totalContent || 0}
            </span>
            ...
            {topCampaigns.map((c, i) => {
              ...
              return (
                <Link
                  ...
                  className="grid grid-cols-[24px_2.2fr_1.6fr_1fr_100px] items-center gap-4 py-3.5 group"
                >
                  <div className="ui-num text-[12px] text-tx-3">
                    {String(i + 1).padStart(2, "0")}
                  </div>
                  <div className="min-w-0">
                    <div className="text-[14px] font-medium text-tx-1 truncate group-hover:text-accent transition-colors">
                      {c.name}
                    </div>
                    {c.project?.name && (
                      <div className="text-[11.5px] text-tx-3 truncate mt-0.5">
                        {c.project.name}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="hidden md:block">
                    <div
                      className={`ui-num text-[17px] font-semibold ${
                        r != null ? "text-accent" : "text-tx-3"
                      }`}
                    >
                      {r != null ? `${r}%` : "—"}
                    </div>
