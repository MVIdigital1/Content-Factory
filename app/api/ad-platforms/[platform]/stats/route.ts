...
async function getGoogleStats(rec: any, days: number, leads: number) {
  ...
  const dateRange =
    days === 7 ? "LAST_7_DAYS" : days === 90 ? "LAST_90_DAYS" : "LAST_30_DAYS";
  ...
  const res = await fetch(
    `https://googleads.googleapis.com/v17/customers/${customerId}/googleAds:search`,
    {
      ...
      body: JSON.stringify({
        query: `SELECT metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.ctr FROM customer WHERE segments.date DURING ${dateRange}`,
      }),
    }
  );
  ...
  const ctr = Number(((Number(m.ctr) || 0) * 100).toFixed(2));
...
async function getMetaStats(rec: any, days: number, leads: number) {
  ...
  const datePreset =
    days === 7 ? "last_7_days" : days === 90 ? "last_90_days" : "last_30_days";
  const accountId = rec.ad_account_id.startsWith("act_")
    ? rec.ad_account_id
    : `act_${rec.ad_account_id}`;

  const res = await fetch(
    `https://graph.facebook.com/v18.0/${accountId}/insights?fields=spend,impressions,clicks,ctr&date_preset=${datePreset}&access_token=${rec.access_token}`
  );
  const json = await res.json();
  if (json.error) throw new Error(json.error.message);
  const d = json.data?.[0] ?? {};
...
export async function GET(
  ...
  const { count } = await supabase
    ...
    .eq("user_id", user.id)
    .eq("source", SOURCE_MAP[platform] ?? platform)
    .gte("created_at", since);
  const leads = count ?? 0;

  try {
    if (platform === "google" && rec.access_token && rec.ad_account_id) {
      return await getGoogleStats(rec, days, leads);
    }
    if (platform === "meta" && rec.access_token && rec.ad_account_id) {
      return await getMetaStats(rec, days, leads);
    }
  } catch {
