...
export default function HealthWidget({
  ...
  const totalPlatform = Object.values(platformCounts).reduce((a, b) => a + b, 0) || 1;

  const successRate = publishedCount > 0
  ...
  return (
    <div className="ui-surface overflow-hidden">
      ...
      <div className="px-5 py-3.5 border-b border-line flex items-center justify-between">
        <div>
          <h2 className="text-[13px] font-semibold text-tx-1">Здоровье контента</h2>
          <p className="text-[11px] text-tx-3 mt-0.5">Активность и охват по каналам</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-line border-b border-line">
        {METRICS.map((m) => (
          <div key={m.label} className="px-5 py-4">
            ...
            <p className="ui-num text-[24px] font-semibold text-tx-1 leading-none">{m.value}</p>
            {m.delta !== null
              ? <DeltaBadge delta={m.delta} />
              : <span className="text-[10px] text-tx-3 mt-1 block">за {period}</span>
            }
          </div>
        ))}
      </div>
      ...
      <div className="px-5 py-4 space-y-3">
        {ALL_PLATFORMS.map((platform) => {
          ...
          return (
            <div key={platform} className="flex items-center gap-3" style={{ opacity: isConnected || isInstagram ? 1 : 0.5 }}>
              <span className="text-[11px] font-medium text-tx-1 capitalize min-w-[76px] flex items-center gap-1.5 flex-shrink-0">
                <span className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                  style={{ background: isConnected ? (PLATFORM_COLOR[platform] || "var(--accent)") : "var(--track)" }} />
                {platform}
              </span>
              <div className="flex-1 h-[3px] rounded-full overflow-hidden" style={{ background: "var(--track)" }}>
                <div className="h-full rounded-full transition-all"
                  style={{ width: `${pct}%`, background: PLATFORM_COLOR[platform] || "var(--accent)" }} />
              </div>
              <div className="flex items-center gap-3 flex-shrink-0 min-w-[160px] justify-end">
                {isConnected ? (
                  <>
                    <span className="text-[10px] text-tx-3"><span className="font-medium text-tx-1">{count}</span> постов</span>
                    <span className="text-[10px] text-tx-3"><span className="font-medium text-tx-1">{published}</span> опубл.</span>
                    <span className="text-[9.5px] font-medium px-2 py-0.5 rounded-[4px]"
                      style={{ background: "var(--accent-dim)", color: "var(--accent)" }}>Активен</span>
                  </>
                ) : isInstagram ? (
                  <>
                    ...
                    <span className="text-[9.5px] font-medium px-2 py-0.5 rounded-[4px]"
                      style={{ background: "rgba(154,103,0,0.1)", color: "var(--c-3)" }}>Ожидает</span>
                  </>
                ) : (
                  <>
